...
import pytan
from pytan.lib.pytan import xml_clean
from pytan.lib import requests
  ...
  <typens:tanium_soap_request {typens}>
    ...
    def authenticate(self, username=None, password=None, session_id=None, **kwargs):
        ...
        if session_id:
            ...
            if persistent:
                ...
                raise pytan.lib.pytan.exceptions.AuthorizationError(m())
        ...
        if not session_id:
            if not self._username:
                raise pytan.lib.pytan.exceptions.AuthorizationError("Must supply username")

            if not self._password:
                raise pytan.lib.pytan.exceptions.AuthorizationError("Must supply password")
        ...
        try:
            ...
            raise pytan.lib.pytan.exceptions.AuthorizationError(m(e))
    ...
    def http_get(self, url, **kwargs):
        ...
        while True:
            try:
                ...
                break
            except pytan.lib.pytan.exceptions.AuthorizationError:
    ...
    def _http_get(self, host, port, url, headers=None, connect_timeout=15,
        ...
        try:
            ...
            raise pytan.lib.pytan.exceptions.HttpError(m(full_url, e))
        ...
        if not response.ok:
            m = "HTTP response: GET request to {!r} returned code: {}, body: {}".format
            raise pytan.lib.pytan.exceptions.HttpError(m(full_url, response.status_code, response_body))
    ...
    def _http_post(self, host, port, url, body=None, headers=None, connect_timeout=15,
        ...
        if not response_body:
            ...
            raise pytan.lib.pytan.exceptions.HttpError(m(full_url))
    ...
    def _check_auth(self):
        ...
        if not self.is_auth:
            ...
            raise pytan.lib.pytan.exceptions.AuthorizationError(err(class_name))
    ...
    def _extract_resultxml(self, response_body):
        ...
        if resultxml_el is None:
            ...
            raise pytan.lib.pytan.exceptions.AuthorizationError(m(response_body))
    ...
    def _get_response(self, request_body, **kwargs):
        ...
        if 'forbidden' in response_command.lower():
            if retry_auth:
                ...
                raise pytan.lib.pytan.exceptions.AuthorizationError(m(response_command))
            ...
            raise pytan.lib.pytan.exceptions.BadResponseError(m(response_command, request_command))
