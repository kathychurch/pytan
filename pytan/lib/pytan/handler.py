...
class Handler(object):
    ...
    def __init__(self, username=None, password=None, host=None, port=443,
        ...
        pytan.lib.pytan.utils.setup_console_logging(gmt_tz=self.gmt_log)

        # create all the loggers and set their levels based on loglevel
        pytan.lib.pytan.utils.set_log_levels(loglevel=self.loglevel)

        # change the format of console logging handler if need be
        pytan.lib.pytan.utils.change_console_format(debug=self.debugformat)
        ...
        if not self.session_id:
            ...
            if not self.username:
                raise pytan.lib.pytan.exceptions.HandlerError("Must supply username!")

            if not self.password:
                raise pytan.lib.pytan.exceptions.HandlerError("Must supply password!")

        if self.password:
            self.password = pytan.lib.pytan.utils.vig_decode(pytan.lib.pytan.constants.PYTAN_KEY, self.password)

        if not self.host:
            raise pytan.lib.pytan.exceptions.HandlerError("Must supply host!")

        if not self.port:
            raise pytan.lib.pytan.exceptions.HandlerError("Must supply port!")

        try:
            self.port = int(self.port)
        except ValueError:
            raise pytan.lib.pytan.exceptions.HandlerError("port must be an integer!")

        pytan.lib.pytan.utils.test_app_port(host=self.host, port=self.port)
    ...
    def read_pytan_user_config(self, kwargs):
        ...
        try:
            ...
            # handle class params
            for h_arg, arg_default in pytan.lib.pytan.constants.HANDLER_ARG_DEFAULTS.iteritems():
                if h_arg not in puc_dict:
                    continue

                if h_arg == 'password':
                    puc_dict['password'] = pytan.lib.pytan.utils.vig_decode(
                        pytan.lib.pytan.constants.PYTAN_KEY, puc_dict['password'],
                    )
    ...
    def write_pytan_user_config(self, **kwargs):
        ...
        puc_dict['password'] = pytan.lib.pytan.utils.vig_encode(pytan.lib.pytan.constants.PYTAN_KEY, self.password)
        ...
        try:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(m(puc, e))
        else:
    ...
    def ask(self, **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.Q_OBJ_MAP` : maps qtype to a method in Handler()
        ...
        q_obj_map = pytan.lib.pytan.utils.get_q_obj_map(qtype=qtype)
    ...
    def ask_saved(self, refresh_data=False, **kwargs):
        ...
        clean_kwargs = pytan.lib.pytan.utils.clean_kwargs(kwargs=kwargs)
        ...
        if len(sq_objs) != 1:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(kwargs, sq_obj_str))
        ...
        if refresh_data:
            ...
            shrunk_obj = pytan.lib.pytan.utils.shrink_obj(obj=sq_obj)
    ...
    def ask_manual(self, **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.FILTER_MAPS` : valid filter dictionaries for filters
        :data:`pytan.lib.pytan.constants.OPTION_MAPS` : valid option dictionaries for options
        ...
        pytan.lib.pytan.utils.check_for_help(kwargs=kwargs)

        sensors = kwargs.get('sensors', [])
        q_filters = kwargs.get('question_filters', [])
        q_options = kwargs.get('question_options', [])

        sensor_defs = pytan.lib.pytan.utils.dehumanize_sensors(sensors=sensors)
        q_filter_defs = pytan.lib.pytan.utils.dehumanize_question_filters(question_filters=q_filters)
        q_option_defs = pytan.lib.pytan.utils.dehumanize_question_options(question_options=q_options)
    ...
    def parse_query(self, question_text, **kwargs):
        ...
        if not self.session.platform_is_6_5(**kwargs):
            ...
            raise pytan.lib.pytan.exceptions.UnsupportedVersionError(m)
    ...
    def ask_parsed(self, question_text, picker=None, get_results=True, **kwargs):
        ...
        if not parse_job_results:
            ...
            raise pytan.lib.pytan.exceptions.ServerParseError(m())
        ...
        if picker is None:
            ...
            raise pytan.lib.pytan.exceptions.PickerError(pw())
    ...
    def deploy_action(self, **kwargs):
        ...
        run : bool, optional
            * default: False
            * False: just ask the question that pertains to verify action, export the results to CSV, and raise pytan.lib.pytan.exceptions.RunFalse -- does not deploy the action
        ...
        action_filter_defs = pytan.lib.pytan.utils.dehumanize_sensors(action_filters, 'action_filters', True)
        action_option_defs = pytan.lib.pytan.utils.dehumanize_question_options(action_options)
        package_def = pytan.lib.pytan.utils.dehumanize_package(package)
    ...
    def stop_action(self, id, **kwargs):
        ...
        if after_action_obj.stopped_flag:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(m(self.export_obj(after_action_obj, 'json')))
    ...
    def get_result_data(self, obj, aggregate=False, shrink=True, **kwargs):
        ...
        if shrink:
            shrunk_obj = pytan.lib.pytan.utils.shrink_obj(obj=obj)
        else:
            shrunk_obj = obj

        kwargs['export_flag'] = pytan.lib.pytan.utils.get_kwargs_int(key='export_flag', default=0, **kwargs)
    ...
    def get_result_data_sse(self, obj, **kwargs):
        ...
        *Version support*
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.SSE_FORMAT_MAP` : maps `sse_format` to an integer for use by the SOAP API
        :data:`pytan.lib.pytan.constants.SSE_RESTRICT_MAP` : maps sse_format integers to supported platform versions
        :data:`pytan.lib.pytan.constants.SSE_CRASH_MAP` : maps platform versions that can cause issues in various scenarios
        ...
        if not poller_success:
            ...
            raise pytan.lib.pytan.exceptions.ServerSideExportError(m(sse_status))
    ...
    def create_from_json(self, objtype, json_file, **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.GET_OBJ_MAP` : maps objtype to supported 'create_json' types
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())

        obj_map = pytan.lib.pytan.utils.get_obj_map(objtype=objtype)

        create_json_ok = obj_map['create_json']

        if not create_json_ok:
            json_createable = ', '.join([
                x for x, y in pytan.lib.pytan.constants.GET_OBJ_MAP.items() if y['create_json']
            ])
            m = "{} is not a json createable object! Supported objects: {}".format
            raise pytan.lib.pytan.exceptions.HandlerError(m(objtype, json_createable))

        add_obj = pytan.lib.pytan.utils.load_taniumpy_from_json(json_file=json_file)
        ...
        ret = pytan.lib.pytan.utils.get_taniumpy_obj(obj_map=all_type)()
        ...
        for x in obj_list:
            try:
                ...
                raise pytan.lib.pytan.exceptions.HandlerError(m(x, e, x.to_json(x)))
    ...
    def run_plugin(self, obj, **kwargs):
        ...
        sql_zipped = pytan.lib.pytan.utils.plugin_zip(p=plugin_result)
    ...
    def get_dashboards(self, name='', **kwargs):
        ...
        if name:
            ...
            if not sql_zipped:
                ...
                raise pytan.lib.pytan.exceptions.NotFoundError(m(name))
    ...
    def create_sensor(self, **kwargs):
        ...
        Raises
        ------
        pytan.lib.pytan.exceptions.HandlerError : :exc:`pytan.lib.pytan.utils.pytan.lib.pytan.exceptions.HandlerError`
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())
        ...
        raise pytan.lib.pytan.exceptions.HandlerError(m)
    ...
    def create_package(self, name, command, display_name='', file_urls=[],
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.FILTER_MAPS` : valid filters for verify_filters
        :data:`pytan.lib.pytan.constants.OPTION_MAPS` : valid options for verify_filter_options
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())
        ...
        metadatalist_obj = pytan.lib.pytan.utils.build_metadatalist_obj(properties=metadata)
        ...
        # VERIFY FILTERS
        if verify_filters:
            verify_filter_defs = pytan.lib.pytan.utils.dehumanize_question_filters(
                question_filters=verify_filters
            )
            verify_option_defs = pytan.lib.pytan.utils.dehumanize_question_options(
                question_options=verify_filter_options
            )
            verify_filter_defs = self._get_sensor_defs(defs=verify_filter_defs, **clean_kwargs)
            add_verify_group = pytan.lib.pytan.utils.build_group_obj(
        ...
        if parameters_json_file:
            add_package_obj.parameter_definition = pytan.lib.pytan.utils.load_param_json_file(
    ...
    def create_group(self, groupname, filters=[], filter_options=[], **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.FILTER_MAPS` : valid filters for filters
        :data:`pytan.lib.pytan.constants.OPTION_MAPS` : valid options for filter_options
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())
        ...
        filter_defs = pytan.lib.pytan.utils.dehumanize_question_filters(question_filters=filters)
        option_defs = pytan.lib.pytan.utils.dehumanize_question_options(question_options=filter_options)
        ...
        add_group_obj = pytan.lib.pytan.utils.build_group_obj(
    ...
    def create_user(self, name, rolename=[], roleid=[], properties=[], group='', **kwargs):
        ...
        metadatalist_obj = pytan.lib.pytan.utils.build_metadatalist_obj(
    ...
    def delete(self, objtype, **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.GET_OBJ_MAP` : maps objtype to supported 'search' keys
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())
        ...
        if not delete_ok:
            deletable = ', '.join([
                x for x, y in pytan.lib.pytan.constants.GET_OBJ_MAP.items() if y['delete']
            ])
            m = "{} is not a deletable object! Deletable objects: {}".format
            raise pytan.lib.pytan.exceptions.HandlerError(m(objtype, deletable))
    ...
    def export_obj(self, obj, export_format='csv', **kwargs):
        ...
        See Also
        --------
        :data:`pytan.lib.pytan.constants.EXPORT_MAPS` : maps the type `obj` to `export_format` and the optional args supported for each
        """
        self._debug_locals(sys._getframe().f_code.co_name, locals())
        ...
        type_match = [
            x for x in pytan.lib.pytan.constants.EXPORT_MAPS if isinstance(obj, getattr(taniumpy, x))
        ]
        ...
        if not type_match:
            ...
            supp_types = ', '.join(pytan.lib.pytan.constants.EXPORT_MAPS.keys())
            raise pytan.lib.pytan.exceptions.HandlerError(err(objtype, supp_types))

        # get the export formats for this obj type
        export_formats = pytan.lib.pytan.constants.EXPORT_MAPS.get(type_match[0], '')

        if export_format not in export_formats:
            err = (
                "{!r} not a supported export format for {}, must be one of: {}"
            ).format(export_format, objclassname, ', '.join(export_formats))
            raise pytan.lib.pytan.exceptions.HandlerError(err)
        ...
        for opt_key in opt_keys:
            ...
            pytan.lib.pytan.utils.check_dictkey(**check_args)
        ...
        if class_handler:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(objclassname))
    ...
    def create_report_file(self, contents, report_file=None, **kwargs):
        ...
        if report_file is None:
            report_file = 'pytan_report_{}.txt'.format(pytan.lib.pytan.utils.get_now())
    ...
    def export_to_report_file(self, obj, export_format='csv', **kwargs):
        ...
        if not report_file:
            report_file = "{}_{}.{}".format(
                type(obj).__name__, pytan.lib.pytan.utils.get_now(), export_format,
            )
    ...
    def get(self, objtype, **kwargs):
        ...
        err_keys = ['pytan_help']
        err_args = pytan.lib.pytan.utils.clean_kwargs(kwargs=kwargs, keys=err_keys)
        ...
        if not api_attrs or (not any(api_kwattrs) and manual_search):
            ...
            for k, v in kwargs.iteritems():
                ...
                if not hasattr(all_objs[0], k):
                    continue
                if not pytan.lib.pytan.utils.is_list(v):
                    v = [v]
            ...
            if not return_objs:
                ...
                raise pytan.lib.pytan.exceptions.HandlerError(err(objtype, err_args))
        ...
        if not any(api_kwattrs):
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(objtype, api_attrs))
        ...
        raise pytan.lib.pytan.exceptions.HandlerError(err(objtype))
    ...
    def get_all(self, objtype, **kwargs):
        ...
        all_type = obj_map['all']
        api_obj_all = pytan.lib.pytan.utils.get_taniumpy_obj(obj_map=all_type)()
    ...
    def _add(self, obj, **kwargs):
        ...
        try:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(search_str, e))
        ...
        try:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(search_str))
    ...
    def _find(self, obj, **kwargs):
        ...
        if pytan.lib.pytan.utils.empty_obj(found):
    ...
    def _get_multi(self, obj_map, **kwargs):
        ...
        api_obj_multi = pytan.lib.pytan.utils.get_taniumpy_obj(obj_map=multi_type)()
        ...
        for k, v in api_kw.iteritems():
            ...
            if pytan.lib.pytan.utils.is_list(v):
                for i in v:
                    api_obj_single = pytan.lib.pytan.utils.get_taniumpy_obj(obj_map=single_type)()
    ...
    def _get_single(self, obj_map, **kwargs):
        ...
        found = pytan.lib.pytan.utils.get_taniumpy_obj(obj_map=all_type)()
    ...
    def _get_sensor_defs(self, defs, **kwargs):
        ...
        s_obj_map = pytan.lib.pytan.constants.GET_OBJ_MAP['sensor']
    ...
    def _get_package_def(self, d, **kwargs):
        ...
        s_obj_map = pytan.lib.pytan.constants.GET_OBJ_MAP['package']
        search_keys = s_obj_map['search']
    ...
    def _export_class_BaseType(self, obj, export_format, **kwargs): # noqa
        ...
        if format_handler:
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(export_format))
    ...
    def _export_format_csv(self, obj, **kwargs):
        ...
        if not hasattr(obj, 'write_csv'):
            ...
            raise pytan.lib.pytan.exceptions.HandlerError(err(obj))
    ...
    def _export_format_xml(self, obj, **kwargs):
        ...
        result = pytan.lib.pytan.utils.xml_pretty(x=raw_xml, **clean_kwargs)
        return result
    ...
    def _deploy_action(self, run=False, get_results=True, **kwargs):
        ...
        package_def = pytan.lib.pytan.utils.parse_defs(
            defname='package_def',
            deftypes=['dict()'],
            empty_ok=False,
            **clean_kwargs
        )
        action_filter_defs = pytan.lib.pytan.utils.parse_defs(
        ...
        action_option_defs = pytan.lib.pytan.utils.parse_defs(
        ...
        pytan.lib.pytan.utils.val_package_def(package_def=package_def)
        pytan.lib.pytan.utils.val_sensor_defs(sensor_defs=action_filter_defs)
        ...
        start_seconds_from_now = pytan.lib.pytan.utils.get_kwargs_int(
            key='start_seconds_from_now', default=0, **clean_kwargs
        )

        expire_seconds = pytan.lib.pytan.utils.get_kwargs_int(key='expire_seconds', **clean_kwargs)
        ...
        if not run:
            ...
            pre_action_sensor_defs = pytan.lib.pytan.utils.dehumanize_sensors(sensors=pre_action_sensors)
            ...
            q_clean_kwargs = pytan.lib.pytan.utils.clean_kwargs(kwargs=kwargs, keys=q_clean_keys)
            ...
            e_clean_kwargs = pytan.lib.pytan.utils.clean_kwargs(kwargs=kwargs, keys=e_clean_keys)
            ...
            raise pytan.lib.pytan.exceptions.RunFalse(m(report_path, len(result)))

        # BUILD THE PACKAGE OBJECT TO BE ADDED TO THE ACTION
        add_package_obj = pytan.lib.pytan.utils.copy_package_obj_for_action(obj=package_def['package_obj'])
        ...
        param_objlist = pytan.lib.pytan.utils.build_param_objlist(
        ...
        if action_filter_defs or action_option_defs:
            targetgroup_obj = pytan.lib.pytan.utils.build_group_obj(
        ...
        if start_seconds_from_now:
            add_obj.start_time = pytan.lib.pytan.utils.seconds_from_now(secs=start_seconds_from_now)
    ...
    def _ask_manual(self, get_results=True, **kwargs):
        ...
        sensor_defs = pytan.lib.pytan.utils.parse_defs(
            defname='sensor_defs',
            deftypes=['list()', 'str()', 'dict()'],
            strconv='name',
            empty_ok=True,
        ...
        q_filter_defs = pytan.lib.pytan.utils.parse_defs(
        ...
        q_option_defs = pytan.lib.pytan.utils.parse_defs(
        ...
        max_age_seconds = pytan.lib.pytan.utils.get_kwargs_int(
            key='max_age_seconds', default=600, **clean_kwargs
        )

        # do basic validation of our defs
        pytan.lib.pytan.utils.val_sensor_defs(sensor_defs=sensor_defs)
        pytan.lib.pytan.utils.val_q_filter_defs(q_filter_defs=q_filter_defs)
        ...
        selectlist_obj = pytan.lib.pytan.utils.build_selectlist_obj(sensor_defs=sensor_defs)

        # build a Group object from our question filters/options
        group_obj = pytan.lib.pytan.utils.build_group_obj(
            q_filter_defs=q_filter_defs, q_option_defs=q_option_defs,
        )

        # build a Question object from selectlist_obj and group_obj
        add_obj = pytan.lib.pytan.utils.build_manual_q(selectlist_obj=selectlist_obj, group_obj=group_obj)
    ...
    def _check_sse_format_support(self, sse_format, sse_format_int, **kwargs):
        ...
        if sse_format_int not in pytan.lib.pytan.constants.SSE_RESTRICT_MAP:
            return

        restrict_maps = pytan.lib.pytan.constants.SSE_RESTRICT_MAP[sse_format_int]
    ...
    def _resolve_sse_format(self, sse_format, **kwargs):
        ...
        sse_format_int = [x[-1] for x in pytan.lib.pytan.constants.SSE_FORMAT_MAP if sse_format.lower() in x]

        if not sse_format_int:
            m = "Unsupport export format {!r}, must be one of:\n{}".format
            ef_map_txt = '\n'.join(
                [', '.join(['{!r}'.format(x) for x in y]) for y in pytan.lib.pytan.constants.SSE_FORMAT_MAP]
            )
            raise pytan.lib.pytan.exceptions.HandlerError(m(sse_format, ef_map_txt))
    ...
    def _check_sse_crash_prevention(self, obj, **kwargs):
        ...
        restrict_maps = pytan.lib.pytan.constants.SSE_CRASH_MAP
    ...
    def _check_sse_timing(self, ok_version, **kwargs):
        """Checks that the last server side export was at least 1 second ago if server version is less than any versions in pytan.lib.pytan.constants.SSE_CRASH_MAP
        ...
        if last_get_rd_sse:
            ...
            if last_elapsed.seconds == 0 and not ok_version:
                ...
                raise pytan.lib.pytan.exceptions.ServerSideExportError(m())
    ...
    def _check_sse_empty_rs(self, obj, ok_version, **kwargs):
        ...
        if not ok_version:
            ...
            if ri.row_count == 0:
                ...
                raise pytan.lib.pytan.exceptions.ServerSideExportError(m(ri))
